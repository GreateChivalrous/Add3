...
//透视投影（一点透视）
void Matrix4x3::setupPerspectiveProject1(float R/*视径*/, float d/*视距*/) {
	m11 = m22 = 1.0f;
...
//透视投影（二点透视）
void Matrix4x3::setupPerspectiveProject2(float R/*视径*/, float d/*视距*/) {
	m11 = sqrt(2) / 2;
	m14 = -sqrt(2) / (2 * d);
	m22 = 1.0f;
	m31 = -sqrt(2) / 2;
	m34 = -sqrt(2) / (2 * d);
	t = R / d;
	m12 = m13 = m21 = m23 = m24 = m32 = m33 = tx = ty = tz = 0.0f;
...
Vector3 operator *(const Vector3& p, const Matrix4x3& m) {
	return Vector3(p.x * m.m11 + p.y * m.m21 + p.z * m.m31 + m.tx,
		...
		p.x * m.m13 + p.y * m.m23 + p.z * m.m33 + m.tz,p.color);
}
